# pip install -r requirements.txt -> harus install
# !sudo apt install tesseract-ocr-ind -> harus install 
import cv2
import pandas as pd
import re
import pytesseract
...
def extract_text(extract):
  keys = ['ktp_nik', 'ktp_name', 'ktp_place_of_birth', 'ktp_birth_date', 'ktp_gender']
  ...
  for word in extract.split("\n"):
    ...
    if re.search("NIK", word):
      ...
      try:
        ...
        for arr in array:
          if len(arr) == 16:
            dict_ktp['ktp_nik'] = str(arr)

      except:
        dict_ktp['ktp_nik'] = None
    ...
    if re.search("Nama", word):
      ...
      try:
        ...
        dict_ktp['ktp_name'] = ' '.join([str(elem) for elem in array])

      except:
        dict_ktp['ktp_name'] = None
    ...
    if re.search(r'[A-Z]+[A-Z]+[,.]+ \d{2}\W\d{2}\W\d{4}', word):
      ...
      try:
        ...
        dict_ktp['ktp_place_of_birth'] = ' '.join([str(elem) for elem in array])
        dict_ktp['ktp_birth_date'] = '/'.join([str(v) for elem in arr for v in re.findall(r"[\w']+", elem)])
        dict_ktp['ktp_birth_date'] = time.mktime(time.strptime(dict_ktp['birth_date'], "%d/%m/%Y"))

      except:
        dict_ktp['ktp_place_of_birth'] = None
        dict_ktp['ktp_birth_date'] = None

    try:
      if re.search("^PEREM|.*PUAN", word):
        dict_ktp['ktp_gender'] = 'Female'
      elif re.search("LAKI", word):
        dict_ktp['ktp_gender'] = 'Male'

    except:
      dict_ktp['ktp_gender'] = None
